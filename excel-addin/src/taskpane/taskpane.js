...
function _build_completion_query_context_string({ topic, fields, completions = [], partial = [], side_info = [] }) {
  ...
  return {
    context_string: `${CONTEXT}${side_info_string}${TOPIC_QUERY}${field_string}`,
    existing_completions: `${completion_string}`,
  };
}
...
async function suggest_fields({ topic, fields = [] }) {
  ...
  // first token)
  const sorted_fields = Object.entries(logits[endheader_idx][0]).sort((a, b) => {
    return b[1] - a[1];
  });
...
function _advance_line(start_idx, texts) {
  ...
  return {
    result: _strip(texts.slice(start_idx, endline_idx - 1).join("")).split(CELL_SEPARATOR),
    next_idx: endline_idx,
  };
}
...
async function suggest_completions({
  ...
  const query_obj = _build_completion_query_context_string({
    topic: topic,
    fields: fields,
  ...
  const response_json = await cached_request({
    context_string: context_string + existing_completions,
    logprobs: 0,
    length: 100,
    temperature: temperature,
  });
...
function completeTable() {
  ...
  Excel.run(async function (context) {
    ...
    var temperature = 0.0;
    if (side_info === null) {
      completions_obj = await suggest_completions({
        topic: topic,
        fields: fields,
        completions: completions,
        partial: partial,
    ...
    for (let i = 0; i < max_tries; i++) {
      ...
      sheet.getRangeByIndexes(range.rowIndex, range.columnIndex, tmp_results.length, range.columnCount).values =
        tmp_results;
      await context.sync();
      ...
      if (i < max_tries - 1) {
        ...
        completions_obj = await suggest_completions({
          topic: topic,
          fields: fields,
          completions: pruned_completions["result"],
          partial: pruned_completions["partial"],
          temperature: temperature,
        });
      }
    }
    if (pruned_completions["result"].length + 1 != range.rowCount) {
      showToast(
        'Could not complete table in one go; this usually means the selected area is too big. You can try clicking "Complete Table" again.'
      );
    }
    return context.sync();
  }).catch(errorHandlerFunction);
